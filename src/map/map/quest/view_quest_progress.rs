            ...
            .filter_map(|q| {
                if !q.done {
                    match QUEST_DB.get(&q.id) {
                        Some(quest) => match quest.states.get(q.state as usize) {
                            Some(state) => match state.rules.last() {
                                Some(rule) => {
                                    let (icon, progress, target) = match rule.name.as_str() {
                                        "TalkedToNpc" | "InputNpc" => {
                                            (QuestRequirementIcon::Talk, 0, 0)
                                        }
                                        "GotItems" => (
                                        ...
                                        "EnterCoord" | "EnterMap" | "LeaveMap" => {
                                            (QuestRequirementIcon::Step, 0, 0)
                                        }
